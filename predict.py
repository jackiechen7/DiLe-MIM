...
from SEG_UNET.train_mcads import *
...
if __name__ == "__main__":
    ...
    test_dataset = Dataset(x_test_dir, y_test_dir, t_size=t_size,need_down=True) # 修改这个
    ...
    def flatten(lst):
        ...
        for x in lst:
            if isinstance(x, (list, tuple, np.ndarray)):
                out.extend(np.ravel(x))
            else:
                out.append(x)
        return out


    dice_flat = flatten(dice_lst)
    iou_flat = flatten(iou_lst)
    hd_flat = flatten(hd_lst)

    if len(dice_flat) > 0:
        print("Average DICE:", np.mean(dice_flat))
        print("Average IoU:", np.mean(iou_flat))
        print("Average HD:", np.mean(hd_flat))
    else:
        print("⚠️ No valid predictions found — lists are empty.")
